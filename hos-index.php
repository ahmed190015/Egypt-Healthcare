...
if (isset($_POST['post'])) {
    ...
    $sql = "INSERT INTO posts (hospital_id, text) VALUES (:hospital_id, :text)";
    $stmt = $db->prepare($sql);
    ...
    header('Location: hos-index.html');
  }

  if (isset($_POST['poll'])) {
    // Get the form data
    $poll_text = $_POST['polltext'];
    $poll_option1 = $_POST['option1'];
    $poll_option2 = $_POST['option2'];

    
    // Insert the form data into the database
    $sql = "INSERT INTO poll (hospital_id, polltext, option1, option2) VALUES (:hospital_id, :text, :option1, :option2)";
    $stmt = $db->prepare($sql);
    ...
    $videoFileName = '';
      ...
      if (!empty($video['tmp_name'])) {
        ...
        if (!empty($imageFileName) || !empty($videoFileName)) {
          // Insert the form data into the database table
          $stmt = $db->prepare("INSERT INTO media (hospital_id,textt, image, vedio) VALUES (:hospital_id, :text, :image, :video)");
          $stmt->bindParam(':hospital_id', $_SESSION['id']);
